...
pub fn handle_update_key_registration(old_key:HashString, signed_old_key:Signature, new_key:HashString, derivation_index: u64, key_type:AppKeyType, context:String ) -> ZomeApiResult<Address> {
...
// get Address of the registered old_key
    ...
    match hdk::update_entry(key_registration_entry, &old_key_address){
        ...
        Err(_)=>{
            Err(ZomeApiError::from("handle_update_key_registration: Unable to Update Key".to_string()))
        }
    }
}
...
fn get_address_of_key(key:&HashString, list:Vec<(ChainHeader,Entry)>)-> Result<HashString,HolochainError> {
    ...
    if !address.is_empty() {
        ...
        Err(HolochainError::ErrorGeneric( format!("get_address_of_key: The values your Searching does not exists")))
    }
}

fn get_address_of_key_meta(key:&HashString, list:Vec<(ChainHeader,Entry)>)-> Result<HashString,HolochainError> {
    let mut address:Vec<HashString>=Vec::new();
    for k in list {
        if let Entry::App(_, json_string) = k.1 {
...
fn query_local_chain_for_entry_type(entry_type: String) -> Result<Vec<(ChainHeader,Entry)>,HolochainError> {
