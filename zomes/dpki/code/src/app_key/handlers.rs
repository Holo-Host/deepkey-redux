...
pub fn handle_register_key(app_dna_hash: String, app_name: String, public_key: HashString) -> ZomeApiResult<Address> {
    let entry = Entry::App("app_key".into(), AppKey::new(&app_dna_hash, &app_name, &public_key).into());
    hdk::commit_entry(&entry)
}

pub fn handle_get_registered_key() -> ZomeApiResult<Vec<AppKey>> {
    let raw_result: Vec<Result<AppKey, JsonError>> = query_local_chain_for_entry_type("app_key".to_string())?
        .iter()
        .map(| (_, entry) | {
            match entry {
                Entry::App(_, data) => {
                    AppKey::try_from(data.to_owned())
                },
                _ => unreachable!()
            }
        })
