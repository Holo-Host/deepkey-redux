...
// Check if the key exist for the authorizor
    const not_registered_key = await liza.app.call("dpki", "key_status", {key:"Not-Registered-Key"})
...
// Lets create an authorizor key
    const updated_authorizor_commit = await liza.app.call("dpki", "set_authorizor", {
      authorization_key_path: 2,
      signed_auth_key:SIGNED_AUTH_KEY_2
    })
    t.ok(updated_authorizor_commit.Ok)

    const checking_new_authorizor_key = await liza.app.call("dpki", "key_status", {key:updated_authorizor_commit.Ok})
    t.deepEqual(checking_new_authorizor_key.Ok,"live" )

    sleep.sleep(5);
// Check if the key exist for the authorizor
    const checking_old_authorizor_key = await liza.app.call("dpki", "key_status", {key:authorizor_commit.Ok})
    t.deepEqual(checking_old_authorizor_key.Ok,"modified" )


    const updated_rule_commit = await liza.app.call("dpki", "update_rules", {revocation_key:"Updated_Revocation--------------Key"})
    t.ok(updated_rule_commit.Ok )

    sleep.sleep(5);
// Check if your getting the right hash
    const my_updated_rules = await liza.app.call("dpki", "get_rules", {})
